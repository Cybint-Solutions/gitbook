...
### [**End User Portal**](../application-reference/users/end-users.md) **Enhancements**
...
* **Recipient Grouping:** Create [recipient groups](../application-reference/users/recipient-groups.md) directly within the [campaign creation wizard](../application-reference/campaigns/create-a-campaign/wizard-mode/) for a streamlined setup.
* [**System-wide Filters**](../application-reference/settings/common-system-settings/filter-settings.md)**:** Manage IP address and User-Agent rules across all campaigns via the UI, centralizing campaign access control and minimizing false positives.
...
* **License Page Redesign:** License-related actions are now neatly organized under clear headings, improving [license management](../application-reference/account-settings/license.md).
* **Notifications Module:** All [system notifications](../application-reference/support/status/notifications.md)—read and unread—are now available in a centralized view, giving administrators easy access to updates, system errors, new templates, and general alerts.
* **Azure OAuth API Permission Updates:** Permissions for Azure OAuth 2.0 have been refined, requesting only OpenID for secure authentication across both [SSO](../application-reference/settings/common-system-settings/sso-configuration.md) and the [XML Reporting plugin](../guides/reporting-plugin/).
